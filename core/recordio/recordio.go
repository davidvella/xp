...
func ReadRecord(r io.Reader) (partition.Record, error) {
	...
	if err := binary.Read(r, binary.LittleEndian, &idLen); err != nil {
		...
		return partition.Record{}, fmt.Errorf("error reading ID length: %v", err)
	}

	// Read ID
	idBytes := make([]byte, idLen)
	if _, err := io.ReadFull(r, idBytes); err != nil {
		return partition.Record{}, fmt.Errorf("error reading ID: %v", err)
	}

	// Read PartitionKey length
	var partitionKeyLen uint32
	if err := binary.Read(r, binary.LittleEndian, &partitionKeyLen); err != nil {
		return partition.Record{}, fmt.Errorf("error reading PartitionKey length: %v", err)
	}

	// Read PartitionKey
	partitionKeyBytes := make([]byte, partitionKeyLen)
	if _, err := io.ReadFull(r, partitionKeyBytes); err != nil {
		return partition.Record{}, fmt.Errorf("error reading PartitionKey: %v", err)
	}

	// Read timestamp
	var unixNano int64
	if err := binary.Read(r, binary.LittleEndian, &unixNano); err != nil {
		return partition.Record{}, fmt.Errorf("error reading timestamp: %v", err)
	}

	// Read timezone length and name
	var timezoneLen uint32
	if err := binary.Read(r, binary.LittleEndian, &timezoneLen); err != nil {
		return partition.Record{}, fmt.Errorf("error reading timezone length: %v", err)
	}

	timezoneBytes := make([]byte, timezoneLen)
	if _, err := io.ReadFull(r, timezoneBytes); err != nil {
		return partition.Record{}, fmt.Errorf("error reading timezone: %v", err)
	}

	// Load the timezone
	loc, err := time.LoadLocation(string(timezoneBytes))
	if err != nil {
		return partition.Record{}, fmt.Errorf("error loading timezone: %v", err)
	}

	// Read data length
	var dataLen uint32
	if err := binary.Read(r, binary.LittleEndian, &dataLen); err != nil {
		return partition.Record{}, fmt.Errorf("error reading data length: %v", err)
	}

	// Read data
	data := make([]byte, dataLen)
	if _, err := io.ReadFull(r, data); err != nil {
		return partition.Record{}, fmt.Errorf("error reading data: %v", err)
	}

	// Read newline
	nl := make([]byte, 1)
	if _, err := io.ReadFull(r, nl); err != nil {
		return partition.Record{}, fmt.Errorf("error reading newline: %v", err)
	}
